...
public class GuessMoMidlet extends MIDlet implements CommandListener {
    ...
    private TextField textF, playerTextField,scoreTextField;
    ...
    private void algo() {
       ...
       total=(container*10)+10;
        if (n < x) {
            ...
            alert = new Alert("Not quite", "Try a smaller value "+ x, null, AlertType.WARNING);
...
//        disp.setCurrent(listOfPlayerScores);        
        ...
        try {
            String newItem = playerTextField.getString() + " - " + total + "pts.";
            byte[] bytes = newItem.getBytes();
            rs.addRecord(bytes, 0, bytes.length);

//            listOfPlayerScores.append(playerTextField.getString(),scoreTextField.getString());
    ...
    void playerScore() {
        ...
        scoreTextField = new TextField("Score",""+total, 25, TextField.UNEDITABLE);
        player.append(scoreTextField);
